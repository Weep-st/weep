...
async function test() {
  ...
  console.log('Result Inactivo:', { data, error });
  ...
  const resActivo = await supabase.from('repartidores').update(updatesActivo).eq('id', driverId);
  console.log('Result Activo:', { data: resActivo.data, error: resActivo.error });
}
