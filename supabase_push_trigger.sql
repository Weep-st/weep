...
BEGIN
  ...
  IF (v_onesignal_id IS NOT NULL AND v_onesignal_id <> '') THEN
    v_push_payload := jsonb_build_object(
      ...
      'message', 'Pedido #' || split_part(NEW.id, '-', 2) || ': Aceptalo o rechazalo en el panel de repartidores',
      'data', jsonb_build_object('pedidoId', NEW.id, 'type', 'new_order')
    );

    -- Configurar headers (IMPORTANTE: Si la función requiere JWT, agregar Authorization: Bearer ...)
