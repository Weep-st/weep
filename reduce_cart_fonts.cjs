...
// 1. Reduce Cart Item Name Font Size
const cartItemRegex = /<span className="cart-item-name" style=\{\{ fontSize: '0.8rem', lineHeight: '1.1' \}\}>\{item\.nombre\}<\/span>/g;
const cartItemReplacement = `<span className="cart-item-name" style={{ fontSize: '0.7rem', lineHeight: '1.1' }}>{item.nombre}</span>`;

if (cartItemRegex.test(content)) {
    content = content.replace(cartItemRegex, cartItemReplacement);
    console.log("Reduced cart item name size");
} else {
    console.log("Could not find cart item name!");
}

// 2. Reduce Upgrade Button Size
const btnRegex = /<button type="button" className="btn btn-success btn-sm animate-fade-in" style=\{\{ borderRadius: '6px', fontWeight: 'bold', fontSize: '0.65rem', padding: '4px 10px', boxShadow: '0 2px 6px rgba\(34, 197, 94, 0.3\)', whiteSpace: 'normal', textAlign: 'center', lineHeight: '1.2', maxWidth: '95%' \}\} onClick=\{\(\) => handleUpgradeItem\(item, upgradeOffer\)\}>/g;
const btnReplacement = `<button type="button" className="btn btn-success btn-sm animate-fade-in" style={{ borderRadius: '6px', fontWeight: '600', fontSize: '0.58rem', padding: '3px 8px', boxShadow: '0 1px 3px rgba(34, 197, 94, 0.3)', whiteSpace: 'normal', textAlign: 'center', lineHeight: '1.1', maxWidth: '95%' }} onClick={() => handleUpgradeItem(item, upgradeOffer)}>`;

if (btnRegex.test(content)) {
    content = content.replace(btnRegex, btnReplacement);
    console.log("Reduced upgrade button size");
} else {
    ...
    if (generalBtnRegex.test(content)) {
        ...
        console.log("Reduced upgrade button size (fuzzy)");
    } else {
