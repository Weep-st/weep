-- ==============================================================================
-- WEEPI - Campaña Mundialista 2026 (Base de Datos & Triggers)
-- ==============================================================================
...
-- Trigger para procesar los puntos al finalizar un partido
...
BEGIN
    IF NEW.estado = 'finalizado' AND NEW.goles_a IS NOT NULL AND NEW.goles_b IS NOT NULL THEN
        ...
        LOOP
            ...
            ELSIF (v_pronostico.pronostico_a > v_pronostico.pronostico_b AND NEW.goles_a > NEW.goles_b) OR
                  (v_pronostico.pronostico_a < v_pronostico.pronostico_b AND NEW.goles_a < NEW.goles_b) OR
                  (v_pronostico.pronostico_a = v_pronostico.pronostico_b AND NEW.goles_a = NEW.goles_b) THEN
                v_puntos := 100;
            END IF;
...
-- RPC para reclamar premio del calendario diario e inyectar crédito real en la billetera
...
-- RPC para simular la apertura de un sobre y entregar 3 figuritas basadas en rareza
...
-- RPC para pegar figurita en el álbum
...
-- RPC para reciclar 3 repetidas a cambio de 1 sobre cerrado
...
BEGIN
    ...
    IF v_c1 IS NULL OR v_c1 < 1 OR v_c2 IS NULL OR v_c2 < 1 OR v_c3 IS NULL OR v_c3 < 1 THEN
...
-- RPC para completar la misión diaria de login
