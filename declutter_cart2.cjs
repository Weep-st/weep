...
// Note: WhatsApp and state were already added/removed because they didn't crash before the error.
...
if (content.includes(couponSearch)) {
    ...
    if (content.includes(crlfCouponSearch)) {
        content = content.replace(crlfCouponSearch, couponReplace);
...
if (upgradeRegex.test(content)) {
    ...
    // If not found, I'll print the part of the code it might be failing on
